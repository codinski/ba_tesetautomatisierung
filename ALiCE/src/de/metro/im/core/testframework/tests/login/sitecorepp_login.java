...
public class sitecorepp_login extends BaseTestConfig {
	
	public sitecorepp_login() {
	...
	public void openWebApp(String url, String expectedPageTitle){
		assertThat(pp.sc_pp_login.open(url)).containsIgnoringCase(expectedPageTitle);
	}
	
	@Test(description="Verify availability of login applet.", dependsOnMethods="openWebApp")
	public void isLoginAppletAvailable(){
		assertThat(pp.sc_pp_login.loginPanelAvailable()).isTrue();
	}
	...
	public void loginUser(String user, String pw){
		pp.sc_pp_login.loginAs(user, pw);
		assertThat(pp.sc_pp_login.getValidationErrorMessage().isEmpty());
	}
	...
	public void positionIsLandingPage(){
		assertThat(pp.sc_pp_land.isMainPanelAvailable()).isTrue();
	}
	
	@Test(description="Ends session by logging out.", dependsOnMethods="positionIsLandingPage")
	public void logout(){
		assertThat(pp.sc_pp_land.logout().loginPanelAvailable()).isTrue();
	}
